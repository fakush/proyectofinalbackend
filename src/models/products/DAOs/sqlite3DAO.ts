...
export class PersistenciaSQLite3 implements ProductBaseClass {
  ...
  constructor() {
    ...
    this.products.schema.hasTable(this.table).then((exists: any) => {
      if (!exists) {
        logger.log.warn('SQLITE: Initializing table "productos"');
        this.products.schema
          ...
          .then(() => {
            ...
            logger.log.info('SQLITE: Done creating table "productos" & Mockup Data');
          });
      }
    });
  }
